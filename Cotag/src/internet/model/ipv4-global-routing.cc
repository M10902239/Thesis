  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
    {
      ...
      uint32_t selectIndex = 0;
      ...
      else 
        {
          ...
          {
            if (dest == address_list1[0] && dest!= "10.1.1.1") // packet
              {
                selectIndex = pri[0] > -1 ? !(pri[0]%2) : 0 ; //pri is global
                // selectIndex = times[0];
                // times[0] = times[0]^1;
              }
              else if (dest == address_list1[1] && dest!= "10.1.1.1")//packet
              {
                selectIndex = pri[1] > -1 ? !(pri[1]%2) : 0 ; //pri is global
                // selectIndex = times[1];
                // times[1] = times[1]^1;
              }
              else if (dest == address_list1[2] && dest!= "10.1.1.1")//packet
              {
                selectIndex = pri[2] > -1 ? !(pri[2]%2) : 0 ; //pri is global
                // selectIndex = times[2];
                // times[2] = times[2]^1;
              }
              else if (dest == address_list1[3] && dest!= "10.1.1.1")//packet
              {
                selectIndex = pri[3] > -1 ? !(pri[3]%2) : 0 ; //pri is global
                // selectIndex = times[3];
                // times[3] = times[3]^1;
              }
               else if (dest == address_list1[4] && dest!= "10.1.1.1")//packet
              {
                selectIndex = pri[4] > -1 ? !(pri[4]%2) : 0 ; //pri is global
                // selectIndex = times[4];
                // times[4] = times[4]^1;
              }
              else if (dest == address_list1[5] && dest!= "10.1.1.1")//packet
              {
                selectIndex = pri[5] > -1 ? !(pri[5]%2) : 0 ; //pri is global
                // selectIndex = times[5];
                // times[5] = times[5]^1;
              }
              else if (dest == address_list1[6] && dest!= "10.1.1.1")//packet
              {
                selectIndex = pri[6] > -1 ? !(pri[6]%2) : 0 ; //pri is global
                // selectIndex = times[6];
                // times[6] = times[6]^1;
              }
              else if (dest == address_list1[7] && dest!= "10.1.1.1")//packet
              {
...
{ 
  ...
  {
    if (address_list1[list] != header.GetDestination() && address_list1[list] == "102.102.102.102")  
    {

      address_list1[list] = header.GetDestination();
      rout = list;
      // std::cout << list  << "  "<< header.GetDestination()<< "\n";
      break;
    }
    else if (address_list1[list] == header.GetDestination())
    {
      rout = list;
      break;
    }
