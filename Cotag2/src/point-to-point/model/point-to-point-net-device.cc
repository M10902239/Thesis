...
bool NC_enable = true;
int de_list = 0;
uint32_t H_label[UserNum] = {0};
uint32_t L_label[UserNum] = {0};
...
PointToPointNetDevice::Receive (Ptr<Packet> packet)
{
  ...
  if ((RxNowNodeID >= userNode) && pktSize == 1474 && NC_enable)
  {
    ...
    // std::cout << "RxNowNodeID = " << RxNowNodeID << "\n";
      ...
      if (iHD.GetPri()%2 == 1)
      {
        H_label[de_list] = m_RecvIncomingLabel;
      }
      else
      {
        L_label[de_list] = m_RecvIncomingLabel;
      }
      // std::cout << "[Rx] IncomingLabel = " << m_RecvIncomingLabel << "\n";
      temp->AddHeader(ppHD);
      // decode成功的
      ...
      else
      {
        ...
        // std::cout << "NC queue = " <<  m_srncdec[de_list].GetEnqueueBufferSize() << "\n";
        // int NextLabelNumber = m_srncdec[de_list].FindPacketNumberWithLabel(m_smallestUnprocessedLabel+10);
        ...
        // int Next_smallestLabelEncodeNum = m_srncdec[de_list].FindPacketEncodedNumberWithLabel(m_smallestUnprocessedLabel+10);
        // std::cout << Simulator::Now ().GetSeconds () << " m_smallestUnprocessedLabel = " << m_smallestUnprocessedLabel << "," << H_label[de_list] << "," << L_label[de_list] << "," << de_list << "\n";
        if ((Simulator::Now ().GetSeconds () > 1.5 && m_smallestUnprocessedLabel < H_label[de_list] && m_smallestUnprocessedLabel < L_label[de_list]) || Simulator::Now ().GetSeconds () < 1.5)
        {
          if (LabelNumber >= m_smallestLabelEncodeNum)  
          {
            // std::cout << "m_smallestUnprocessedLabel = " << m_smallestUnprocessedLabel << "," << de_list << "\n";
            // std::cout <<" De_List =" << de_list << "\n";
...
bool b[10] = {0};
// uint32_t EncodeMaxNum = 14;  // 1 is PD
  ...
  uint16_t protocolNumber)
{
  ...
  {                                                                         //NowNodeID == 0的原因是從0到1的途中encode
    ...
    {
      if (address_list[list] == ipv4header.GetDestination())
      {
      ...
      else if (address_list[list] != ipv4header.GetDestination() && address_list[list] == "102.102.102.102")  
      {
        address_list[list] = ipv4header.GetDestination();
        NCpreprocess(packet,list);
        break;
      }
    }
