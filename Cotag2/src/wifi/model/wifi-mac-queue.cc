  ...
  if (QueueBase::GetNPackets () < GetMaxSize ().GetValue ())
    {
      ...
      if (item->GetPacket()->GetSize () == 1480)
      {
        ...
        if (pri%2 ==1 && QueueBase::GetNPackets () > 40)
        {
          ofstream fout;
          fout.open( "2_AP1_queue.txt" ,ios::app);
          fout << Simulator::Now ().GetSeconds () <<  " "  << m_RecvIncomingLabel << "\n";
          fout.close();
        } 
        else if (pri%2 == 0 && QueueBase::GetNPackets () > 40)
        {
          if (QueueBase::GetNPackets () != 17)
          {
            ofstream fout;
