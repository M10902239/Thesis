...
uint32_t port = 9999;
double simulationTime = 20.0;                   /* Simulation time in seconds. */
...
CalculateSnr (Ptr<DmgStaWifiMac> staWifiMac, Ptr<DmgWifiMac> apwifiMac)
{
  ...
  // if (apwifiMac == apWifiMac)
  // {
  //   ofstream fout;
  //   fout.open( "2_SNR_AP1.txt" ,ios::app);
  //   fout << Simulator::Now ().GetSeconds () <<  " "  << staSNR << "\n";
  //   fout.close();
  // }
  // else if (apwifiMac == ap2WifiMac)
  // {
  //   ofstream fout;
  //   fout.open( "2_SNR_AP2.txt" ,ios::app);
  //   fout << Simulator::Now ().GetSeconds () <<  " "  << staSNR << "\n";
  //   fout.close();
  // }
...
main (int argc, char *argv[])
{
  ...
  fout.close();
  fout.open( "2_AP1_queue.txt" ,std::ios::trunc);
  fout.close();
  fout.open( "2_AP2_queue.txt" ,std::ios::trunc);
  fout.close();
  fout.open( "0_packet_latency.txt" ,std::ios::trunc);
  fout.close();
   fout.open( "2_SNR_AP1.txt" ,std::ios::trunc);
  fout.close();
   fout.open( "2_SNR_AP2.txt" ,std::ios::trunc);
  fout.close();
  ...
  Config::SetDefault ("ns3::WifiMacQueue::MaxDelay", TimeValue (MilliSeconds (15)));
  ...
  p2pHelper.SetDeviceAttribute ("DataRate", DataRateValue (DataRate ("10Gb/s")));
  ...
  p2pHelper2.SetDeviceAttribute ("DataRate", DataRateValue (DataRate ("10Gb/s")));
  ...
  NetDeviceContainer s2ap1 = p2pHelper2.Install (n1n2);
  NetDeviceContainer s2ap2 = p2pHelper2.Install (n1n3);
...
//============================================================================
  ...
  // positionAlloc->Add (Vector (0.0 , 5.0, 0.0));  // PCP/AP_2
  // positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  // DMG STA 
  // positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  // DMG STA_2 

  // positionAlloc->Add (Vector (1.0 , 0.0, 0.0));  // DMG STA_3 
  // positionAlloc->Add (Vector (1.0 , 0.0, 0.0));  // DMG STA_4 

  // positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  //UserNode
  // positionAlloc->Add (Vector (1.0 , 0.0, 0.0));  //User2Node
  ...
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  // DMG STA_1 
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  // DMG STA_2 
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  // DMG STA_3
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  // DMG STA_4 
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  // DMG STA_5
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  // DMG STA_6 
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  //p2pEndNode
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  //p2pEndNode2
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  //p2pEndNode3
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  //p2pEndNode
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  //p2pEndNode2
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  //p2pEndNode3
  positionAlloc->Add (Vector (-1.0 , 0.0, 0.0));  //p2pEndNode
 ...
 for (uint32_t i = 0; i < UserNum; i++)
  ...
  // Simulator::Schedule (Seconds (2.1+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(22.0 ,0.0,0.0)); //Sector Switch time 1
  // Simulator::Schedule (Seconds (2.1+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(22.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.15+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(0.5 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.15+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(0.5 ,0.0,0.0));

  // Simulator::Schedule (Seconds (2.2+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(17.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.2+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(17.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.3+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(7.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.3+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(7.0 ,0.0,0.0));

  // Simulator::Schedule (Seconds (2.4+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(10.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.4+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(10.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.45+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(10.5 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.45+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(10.5 ,0.0,0.0));

  // Simulator::Schedule (Seconds (2.6+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-18.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.6+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-18.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.7+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-6.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.7+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-6.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.8+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-12.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.8+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-12.0 ,0.0,0.0));

  // Simulator::Schedule (Seconds (2.9+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-11.5 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.9+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-11.5 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.95+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-11.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (2.95+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-11.0 ,0.0,0.0));

  // Simulator::Schedule (Seconds (3.1+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-5.5 ,0.0,0.0));
  // Simulator::Schedule (Seconds (3.1+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-5.5 ,0.0,0.0));
  // Simulator::Schedule (Seconds (3.15+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-12.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (3.15+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-12.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (3.2+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-10.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (3.2+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-10.0 ,0.0,0.0));

  // Simulator::Schedule (Seconds (3.3+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(0.0 ,0.0,0.0));
  // Simulator::Schedule (Seconds (3.3+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(0.0 ,0.0,0.0));
  //實驗1================================================================================================
  Simulator::Schedule (Seconds (1.4+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(0.5 ,0.0,0.0));
  Simulator::Schedule (Seconds (1.4+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(0.5 ,0.0,0.0));

  Simulator::Schedule (Seconds (2.4+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(15.0 ,0.0,0.0)); //Sector Switch time 1
  Simulator::Schedule (Seconds (2.4+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(15.0 ,0.0,0.0));
  Simulator::Schedule (Seconds (2.45+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(0.5 ,0.0,0.0));
  Simulator::Schedule (Seconds (2.45+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(0.5 ,0.0,0.0));

  Simulator::Schedule (Seconds (5.4+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(15.0 ,0.0,0.0)); //Sector Switch time 2
  Simulator::Schedule (Seconds (5.4+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(15.0 ,0.0,0.0));
  Simulator::Schedule (Seconds (5.45+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(0.5 ,0.0,0.0));
  Simulator::Schedule (Seconds (5.45+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(0.5 ,0.0,0.0));

  Simulator::Schedule (Seconds (7.95+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(14.0 ,0.0,0.0)); //Sector Switch time 3
  Simulator::Schedule (Seconds (7.95+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(14.0 ,0.0,0.0));
  Simulator::Schedule (Seconds (8.0+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(0.5 ,0.0,0.0));
  Simulator::Schedule (Seconds (8.0+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(0.5 ,0.0,0.0));

  Simulator::Schedule (Seconds (9.5+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-0.5 ,0.0,0.0)); //go back
  Simulator::Schedule (Seconds (9.5+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-0.5 ,0.0,0.0));

  Simulator::Schedule (Seconds (11.0+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-14.0 ,0.0,0.0)); //Sector Switch time 3
  Simulator::Schedule (Seconds (11.0+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-14.0 ,0.0,0.0));
  Simulator::Schedule (Seconds (11.05+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-0.5 ,0.0,0.0));
  Simulator::Schedule (Seconds (11.05+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-0.5 ,0.0,0.0));

  Simulator::Schedule (Seconds (13.6+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-15.0 ,0.0,0.0)); //Sector Switch time 3
  Simulator::Schedule (Seconds (13.6+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-15.0 ,0.0,0.0));
  Simulator::Schedule (Seconds (13.65+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-0.5 ,0.0,0.0));
  Simulator::Schedule (Seconds (13.65+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-0.5 ,0.0,0.0));

  Simulator::Schedule (Seconds (16.5+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-15.0 ,0.0,0.0)); //Sector Switch time 3
  Simulator::Schedule (Seconds (16.5+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-15.0 ,0.0,0.0));
  Simulator::Schedule (Seconds (16.55+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(-0.5 ,0.0,0.0));
  Simulator::Schedule (Seconds (16.55+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(-0.5 ,0.0,0.0));
