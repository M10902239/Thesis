...
string condition_file = condition ==1 ? "S1" : "S4"; // S1 S2
...
string dataRate[6] = {"100Mbps","100Mbps","300Mbps","300Mbps","500Mbps","500Mbps"};
double tcpbuf[6] = {0.15,0.15,0.45,0.45,0.75,0.75};
...
InstallApplications (Ptr<Node> apNode, Ptr<Node> staNode, Ipv4Address address, uint32_t i)
{
  ...
  Config::SetDefault ("ns3::TcpSocket::SndBufSize", UintegerValue (tcpbuf[i] * 1024 * 1024));//Mbytes
  Config::SetDefault ("ns3::TcpSocket::RcvBufSize", UintegerValue (tcpbuf[i] * 1024 * 1024));
  ...
  src.SetAttribute ("DataRate", DataRateValue (DataRate (dataRate[i])));
...
else
{
  ...
  // cmd.AddValue ("dataRate", "Application data rate", dataRate[i]);
