...
string condition_file = condition ==1 ? "S1" : "S2"; // S1 S2
...
int seedwrite = seed;
string file_des = "./2_rawdata/seed"+ std::to_string(seedwrite) + "/"+ condition_file +"/" + method + method1 + "/" + std::to_string(forflie_UserNum) + "/";
...
uint32_t port = 9999;
double simulationTime = condition == 1 ? 20.0 : 4.0;                  /* Simulation time in seconds. */
...
else
{
  ...
  p2pHelper.SetDeviceAttribute ("DataRate", DataRateValue (DataRate ("3Gb/s")));
...
//============================================================================
  ...
  else if (condition)
  {
    ...
    positionAlloc->Add (Vector (0.0 , 9.0, 0.0));  //p2pGw1Node
    positionAlloc->Add (Vector (0.0 , 9.0, 0.0));  //p2pGwNode
    positionAlloc->Add (Vector (2.0, 5.0, 0.0));   // PCP/AP 
    positionAlloc->Add (Vector (2.0 , 5.0, 0.0));  // PCP/AP_2
 ...
 for (uint32_t i = 0; i < UserNum; i++)
  ...
  else if(condition == 1)
  {
    ...
    Simulator::Schedule (Seconds (7.6+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(8.0 ,0.0,0.0)); //Sector Switch time 1
    Simulator::Schedule (Seconds (7.6+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(8.0 ,0.0,0.0));
    Simulator::Schedule (Seconds (7.65+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(10.0 ,0.0,0.0)); //Sector Switch time 1
    Simulator::Schedule (Seconds (7.65+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(10.0 ,0.0,0.0));
    Simulator::Schedule (Seconds (7.7+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(0.5 ,0.0,0.0));
    Simulator::Schedule (Seconds (7.7+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(0.5 ,0.0,0.0));

    Simulator::Schedule (Seconds (14.4+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(12.0 ,0.0,0.0)); //Sector Switch time 2
    Simulator::Schedule (Seconds (14.4+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(12.0 ,0.0,0.0));
    Simulator::Schedule (Seconds (14.45+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(6.0 ,0.0,0.0));
    Simulator::Schedule (Seconds (14.45+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(6.0 ,0.0,0.0));
    Simulator::Schedule (Seconds (14.50+timeoffset), &SetVelocity, staWifiNode[0] ,Vector(0.5 ,0.0,0.0));
    Simulator::Schedule (Seconds (14.50+timeoffset), &SetVelocity, staWifiNode[1] ,Vector(0.5 ,0.0,0.0));
